...
if (! empty($post_data["billing_phone"])) {
    $post_data["billing_phone"] = preg_replace("/[^0-9]/", "", $post_data["billing_phone"]);
}
...
if (! empty($_SESSION["mobile_app_user_data"])) {
...
if (! $user_id) {
    $include_popup[] = "signup_form";
}
if ($_SESSION["boosted_no_results"]) {
    $include_popup[] = "ris-no-boosted";
}
...
if (! empty($payment_session)) {
    ...
    if ($_SESSION["tokens"][ $token ]["payment_token"] != $payment_token) {
        $token = "";
    }
    if (! empty($_SESSION["tokens"][ $token ]["upsell_membership_selected"])) {
        $post_data["plan_id"] = "upsell";
    }
} else {
    $payment_token = "";
}
...
if (! empty($token) && ! empty($_SESSION["tokens"][ $token ])) {
    ...
    if (empty($token_session_data["billing_lastname"]) || empty($token_session_data["billing_firstname"]) || ( ! empty($membership["show_phone_number"]) && empty($token_session_data["billing_phone"]) )) {
        $token_session_data = array_merge($token_session_data, array_filter($post_data));
    }
    ...
    if (! empty($membership["allowed_payment_gateways"])) {
        ...
        foreach ($membership["allowed_payment_gateways"] as $_pg) {
            $pg_enabled[$_pg] = true;
        }
        unset($_pg);
    ...
    // Braintree
    if ($braintree_payment && empty($token_session_data["bt_client_token"])) {
        $token_session_data["bt_client_token"] = $braintree->get_client_token();
    }
    ...
    if (! empty($user_data["email"]) && ! in_array($user_data["email"], ["jhaney@ualberta.ca", "asdasd@appearen.com", "asitha@socialcatfish.com"])) {
        ...
        if (! empty($token_session_data["section"]) && $token_session_data["section"] == "black-friday-2020") {
    ...
    if ($user_id) {
        ...
        if (!isset($_SESSION["regular_premium_combine"])) {
            if (! preg_match("/^ris|initial|hidden|black-friday-2020|promo\$/im", $membership["type"])) {
                SYSTEM::redirect(RELATIVE_URL . "membership-levels/");
            }
        }
    }
    ...
    if ("checkout" == $action) {
        ...
        if ($multiple_membership_options) {
            ...
            if (! empty($input_get->p_id) && empty($post_data["plan_id"])) {
                $post_data["plan_id"] = $input_get->p_id;
            }
        ...
        switch ($payment_method) {
            ...
            default:
                    $payment_gateway = $payment_gateway_default;
        }

        $is_subscription = ( $membership["recurring_amount"] > 0 );

        if (empty($avoid_validation) && empty($user_data["membership_plan"])) {
            $error_fields = SYSTEM::form_validate($form_validation);
        }
        if (! $user_id && ! empty($post_data["email"]) && User::get_by_email($post_data["email"])) {
            $error_messages[] = "E-Mail Address already in use. Please login.";
        }
        //if ( ! $user_id && ! empty( $post_data["email"] ) && User::is_email_blocked( $post_data["email"] ) ) $error_messages[] = "E-Mail Address blocked.";
        if (empty($post_data["tos_agree"])) {
            $error_messages[] = "Please accept the Terms of Service.";
        }
        ...
        if (! empty($membership["check_mailing_list"]) && defined("SENDY_LIST_{$membership["check_mailing_list"]}")) {
            if (! Membership::check_email_for_sendy_list(constant("SENDY_LIST_{$membership["check_mailing_list"]}"), $post_data["email"])) {
                ...
                $do_not_add_to_sendy_lists = true;
            }
        }
        ...
        if (empty($error_messages)) {
            if (empty($avoid_merge)) {
                $token_session_data = array_merge($token_session_data, SYSTEM::construct_custom_array($form_validation, "name", "{value}"));
            }
            ...
            if ($is_subscription) {
                switch ($membership["start_recurring_period"]) {
                    case "days":
                        if (5 == $membership["start_recurring_frequency"] && ( "payflowpro" == $payment_gateway || "paypal_express_checkout" == $payment_gateway )) {
                            $start_subscription = strtotime("+6 days");
                        } else {
            ...
            if (! $testcase_is_running && $membership["initial_amount"] > 0) {
                if ($payment_gateway == "usaepay") {
                    require("payment_gateways/process_signup.php");
                } elseif ($payment_gateway == "pcb") {
                    if (! in_array($membership["id"], $one_time_premium_plans)) {
                        $payment_error = "Sorry! Subscriptions cannot be purchased using Premium Credit Balance.";
                    } elseif ($membership["initial_amount"] > $user_data["premium_credit_balance"]) {
                        $payment_error = "Sorry! There's not enough credits to purchase this plan.";
                    } else {
            ...
            if (! empty($payment_error)) {
                ...
                if (! $testcase_is_running) {
                    ...
                    if (isset($_SESSION["removed-head-foot"])) {
                        if ($_SESSION["removed-head-foot"]) {
                            $_SESSION["removed-head-foot"] = false;
                        }
                    ...
                    if ($membership["id"] == PLAN_PREMIUM_DATA_MONTHLY || $membership["id"] == PLAN_PREMIUM_DATA) {
                        if (isset($_SESSION["no_premium"])) {
                            $_SESSION["no_premium"]->track_event("conversion", $device);
                        }
                    }

                    if (isset($_SESSION["rebuild_two"])) {
                        $_SESSION["rebuild_two"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["ss_ultra_focus"])) {
                        $_SESSION["ss_ultra_focus"]->track_event("conversion", $device);
                    }
                    ...
                    if (isset($_SESSION["better_results"])) {
                        $_SESSION["better_results"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["br_survey"])) {
                        $_SESSION["br_survey"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["br_no_survey"])) {
                        $_SESSION["br_no_survey"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["existing_user"])) {
                        $_SESSION["existing_user"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["homepage_headline"])) {
                        $_SESSION["homepage_headline"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["ab_blog_2020"])) {
                        $_SESSION["ab_blog_2020"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["ab_ris_speed"])) {
                        $_SESSION["ab_ris_speed"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["ab_us_only"])) {
                        $_SESSION["ab_us_only"]->track_event("conversion", $device);
                    }

                    if (isset($_SESSION["step_four"]) && isset($_SESSION["ab_baselines_old_img"])) {
                        $_SESSION["ab_baselines_old_img"]->track_event("ris_user_conversion", SYSTEM::get_device_type());
                    }

                    if (isset($_SESSION["step_four_basic"]) && isset($_SESSION["ab_baselines_basic_img"])) {
                        $_SESSION["ab_baselines_basic_img"]->track_event("ris_basic_user_conversion", SYSTEM::get_device_type());
                    }
                ...
                if (! $user_id) {
                    ...
                    if ($user_data["membership_plan"]) {
                        User::update($user_id, [ "membership_plan" => null ]);
                    }
                ...
                if ($membership["id"] == PLAN_UNLIMITED_GENERAL_5_DAY_TRIAL) {
                    if ($pause_regular30_is_active) {
                        Membership::cancel_subscription($user_id, $active_memberships[ PLAN_AVOID_PAUSE_REGULAR_30DAYS ], "", null, $cancelled_by_id);
                    }
                    if ($pause_regular60_is_active) {
                        Membership::cancel_subscription($user_id, $active_memberships[ PLAN_AVOID_PAUSE_REGULAR_60DAYS ], "", null, $cancelled_by_id);
                    }
                    if ($pause_regular90_is_active) {
                        Membership::cancel_subscription($user_id, $active_memberships[ PLAN_AVOID_PAUSE_REGULAR_90DAYS ], "", null, $cancelled_by_id);
                    }
                }
                ...
                if ($membership["id"] == PLAN_UNLIMITED_IMAGE_5_DAY_TRIAL) {
                    if ($pause_ris30_is_active) {
                        Membership::cancel_subscription($user_id, $active_memberships[ PLAN_AVOID_PAUSE_REGULAR_30DAYS ], "", null, $cancelled_by_id);
                    }
                    if ($pause_ris60_is_active) {
                        Membership::cancel_subscription($user_id, $active_memberships[ PLAN_AVOID_PAUSE_REGULAR_60DAYS ], "", null, $cancelled_by_id);
                    }
                    if ($pause_ris90_is_active) {
                        Membership::cancel_subscription($user_id, $active_memberships[ PLAN_AVOID_PAUSE_REGULAR_90DAYS ], "", null, $cancelled_by_id);
                    }
                }
                ...
                if (true) {
                    ...
                    if (PLAN_PREMIUM_DATA == $membership["id"]) {
                        $welcome_confirmation_template = "welcome-confirmation-premiumdata";
                    } elseif (PLAN_PREMIUM_DATA_MONTHLY == $membership["id"]) {
                        $welcome_confirmation_template = "welcome-confirmation-premiumdata-monthly";
                    } elseif (PLAN_RIS_BOOSTED_UNLIMITED == $membership["id"]) {
                        $welcome_confirmation_template = "welcome-boosted-unlimited";
                    } elseif (PLAN_DR_PHIL_TRIAL_PLAN == $membership["id"]) {
                        $welcome_confirmation_template = "welcome-confirmation-dr-phil-free";
                    } elseif (PLAN_CRIMINAL_REPORT == $membership["id"] || PLAN_CRIMINAL_RECORDS_ADDON == $membership["id"]) {
                        $welcome_confirmation_template = "welcome-confirmation-criminal-records";
                    }
                    ...
                    if (preg_match("/^avoid-cancel|avoid_pause/i", $membership["type"])) {
                        ...
                        } elseif (preg_match("/^avoid_pause/i", $membership["type"])) {
                    ...
                    if (IN_DEPTH_SEARCH_MEMBERSHIP_ID_NEW == $membership["id"]) {
                        if (empty($cc_email)) {
                            $cc_email = ! empty($post_data["email"]) ? $post_data["email"] : "";
                        }
                        if (empty($cc_billing_phone)) {
                            $cc_billing_phone = ! empty($post_data["billing_phone"]) ? $post_data["billing_phone"] :  $_SESSION["specialist_number"];
                        }
                        if (empty($cc_billing_firstname)) {
                            $cc_billing_firstname = ! empty($post_data["billing_firstname"]) ? $post_data["billing_firstname"] : "";
                        }
                        if (empty($cc_billing_lastname)) {
                            $cc_billing_lastname = ! empty($post_data["billing_lastname"]) ? $post_data["billing_lastname"] : "";
                        }
                ...
                if ($privacy_lock_activated) {
                    PWNED::save_user_pawned_data($cc_email);
                }
                ...
                if (! empty($amazon_pay_ab_test)) {
                    if ($payment_gateway == "amazonpay") {
                        $amazon_pay_ab_test->track_event("conversion_amazon_pay");
                    } else {
                        $amazon_pay_ab_test->track_event("conversion");
                    }
                }
    ...
    if ("initial" == $section && $user_has_search_tokens) {
        $section = "initial";
    } elseif ("ris" == $section && $user_has_image_tokens) {
        $section = "initial";
    } elseif ("upgrade" == $section && empty($user_id)) {
        SYSTEM::redirect(RELATIVE_URL);
    } elseif ("upgrade" == $section) {
        $section = "initial";
    }
    ...
    if ("phone_teaser" == $section) {
        if ($user_id) {
            SYSTEM::redirect(PAGE_URL_REVERSE_PHONE_SEARCH);
        }
    ...
    if (! empty($promo) && ( "black-friday-2020-all-unlimited" == $promo || "black-friday-2020-all-350" == $promo )) {
        if ("black-friday-2020-all-unlimited" == $promo) {
            $promo_id = PLAN_BLACK_2020_FRIDAY_ALL_UNLIMITED;
        }
        if ("black-friday-2020-all-350" == $promo) {
            $promo_id = PLAN_BLACK_2020_FRIDAY_ALL_350;
        }

        foreach ($membership_levels as $levels => $level) {
            if ($level["id"] <> $promo_id) {
                unset($membership_levels[ $levels ]);
            }
        }
    ...
    foreach ($membership_levels as $levels => $level) {
        if ($level["id"] == PLAN_UNLIMITED_MONTHLY_1) {
            unset($membership_levels[ $levels ]);
        }
    }
    ...
    if ($user_id) {
        ...
        foreach ($membership_levels as &$_plan) {
            if ($_plan["purchased"] = ( ( ! empty($active_memberships[ $_plan["id"] ]) || ! empty($active_memberships[ PLAN_UNLIMITED_3999 ]) || ! empty($active_memberships[ PLAN_UNLIMITED_3999_5_DAY_TRIAL ]) ) && $_plan["id"] != IN_DEPTH_SEARCH_MEMBERSHIP_ID_NEW ) ? 1 : 0) {
                $_plan["default"] = 0;
            } elseif (! $default_plan) {
                $default_plan = true;
                $_plan["default"] = 1;
    ...
    foreach ($membership_levels as $index => $membership_level) {
        ...
        if (count($membership_level["features"]) > $max_features_count) {
            $max_features_count = count($membership_level["features"]);
        }
    }
...
if ($settings["indepth_waiting_list"] == "1" && $membership["id"] == IN_DEPTH_SEARCH_MEMBERSHIP_ID_NEW) {
    SYSTEM::redirect(BASE_URL . "search-specialist-waitlist/");
}

if ($membership["id"] == IN_DEPTH_SEARCH_MEMBERSHIP_ID_NEW) {
    $include_popup[] = "phone_required";
}
