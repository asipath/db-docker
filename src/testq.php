    ...
    if ($get_user_plans = Membership::get_user_plans($user_id, true)) {
        foreach ($get_user_plans as $row) {
            switch ($row["start_recurring_period"]) {
                ...
                case "days":
                    time() > strtotime($row["date"] . " +{$row["start_recurring_frequency"]} days") ? $active_plans_non_trial[] = $row : "";
                    break;
                case "mont":
                    time() > strtotime($row["date"] . " +{$row["start_recurring_frequency"]} months") ? $active_plans_non_trial[] = $row : "";
                    break;
                case "year":
                    time() > strtotime($row["date"] . " +{$row["start_recurring_frequency"]} years") ? $active_plans_non_trial[] = $row : "";
                    break;
