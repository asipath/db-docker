        ...
        switch( $cmd ) {
            ...
            case 'membership_signup':
                ...
                if ( $post_data["pg_type"] != "nmi" || ( $post_data["pg_type"] == "nmi" && empty( $datauser ) ) ){
                    ...
                    if( ! empty( $post_data["billing_firstname"] ) ) Behavior::add_user_data("firstname", $post_data["billing_firstname"]);
                    if( ! empty( $post_data["billing_lastname"] ) ) Behavior::add_user_data("lastname", $post_data["billing_lastname"]);
                    if( ! empty( $post_data["email"] ) ) Behavior::add_user_data("email", $post_data["email"]);
                    ...
                    if ( ! $user_id && User::is_email_blocked( $post_data["email"] ) ) $error_messages[] = "E-Mail Address blocked.";

                    if ( $post_data["email"] != $post_data["email_confirm"] ) $error_messages[] = "E-Mail Address does not match.";

                    if( ! empty ( $post_data["show_phone_number"] ) && $post_data["show_phone_number"] == 1 && in_array( "billing_phone", $error_fields ) )  array_unshift( $error_messages, "Phone Number required and must be atleast 10 digits." );
                        ...
                        if ( ! empty( $_SESSION["tokens"][ $token ] ) ) {
                            ...
                            if ( $is_subscription ) {
                                ...
                                switch ( $membership["start_recurring_period"] ) {
                                    ...
                                    case "days": $start_subscription = strtotime("+{$membership["start_recurring_frequency"]} days"); break;
            ...
            case 'paymentcard':
                ...
                if( ! empty( $customer_id ) ) {
                    ...
                    if( ! empty( $user_cards['card_list'] ) ){
                        ...
                        $return['status'] = 'err';
                        if( 'del' == $type  && count( $user_cards['card_list'] ) > 1 ) $return = $stripe->delete_card( $customer_id, $id );
                        else if( 'edit' == $type && count( $user_cards['card_list'] ) >= 1 ){
                        ...
                        if( $return['status'] == 1 ) $ajax_status["status"] = 1;
                        else if( $return['status'] == 'err' ) $ajax_status["status"] = 'err';
                            else $ajax_status["status"] = 2;

                    } else $ajax_status["status"] = 'err';
            ...
            case 'no_result_newsletter_revised':
                ...
                case 'unclaimed_check':
                    ...
                    if(!empty($_SESSION["unclaimed-fund-landing"]["status"]) && $_SESSION["last_search_type"]==SEARCH_TYPE_NAME){
                        ...
                        /** Check if empty **/
                        foreach($last_search_params as $last_search_param) if(empty($last_search_param)) die();
                ...
                case 'pwned_email_list':
                    ...
                    foreach( $emails as $key => $res_email ) {
                        ...
                        if ( $pwned_obj) $result->pwned_emails = true;
            ...
            case 'js_error':
                ...
                if( ! empty( $data["error"] ) ) Behavior::system_log_action( __FILE__, __LINE__, __METHOD__, "JS error=" . $data["error"] . "error_file=" . $data["file"], ["errors", "JS error", "{$data["file"]}::{$data["error"]}" ] );
            ...
            case 'behavior':
                ...
                } else if( $section == "heatmap_run_filter" ){
                ...
                } else if( $section == "heatmap_topbar_disable") {
                ...
                } else if( $section == "heatmap_topbar") {
                ...
                } else if( array_key_exists( $input_post->type, $behavior_actions ) && $input_post->_v == $visitor_key ){
            ...
            case 'chat':
                    if ( trim($post_data["message"]) == "" && ( count($_FILES["file"]["tmp_name"]) == 0 ) ) die();
            ...
            case "faq_cancel_subscription_unset":
                ...
                if( isset ( $_SESSION["faq_cancel_subscription"] ) ) unset( $_SESSION["faq_cancel_subscription"] );
                $ajax_status["status"] = true;
                break;
            ...
            case "get_ris_current_image_count":
                ...
                foreach ( $result as &$image ) {
                    ...
                    if ( ! empty( $image["flags"] ) && ( $image["flags"] & RESULT_FLAG_FACIAL_RECOGNITION ) ) {
                        ...
                        if ( $image["score"] >= SCORE_EXACT_MATCH ) $duplicates[] = $image;
                        else $possible_matches[] = $image;
            ...
            case "unset_regular_premium_combine":
                if( isset ( $_SESSION["regular_premium_combine"] ) ) unset( $_SESSION["regular_premium_combine"] );
                $ajax_status["status"] = true;   
            ...
            case "unset_regular_premium_combine_plan":
                if( isset ( $_SESSION["active_boosted_ontime"] ) ) unset( $_SESSION["active_boosted_ontime"] );
                $ajax_status["status"] = true;              
                break;
            ...
            case "privacy_lock_rating":
                ...
                if( strtotime( $user_data["created"] ) < strtotime( "2022-10-15" ) ) $new_user = 0;
                else {
                    $new_user = 1;
                    $q4 = "";
                    $q5 = "";
			...
			case "chatbot_submit":
				...
				if ( "welcome" == $input_post->data ) {
					...
					if ( ! empty( $chatbot_data["abuse_system"] ) ) break;
