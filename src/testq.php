...
if ($post_data["gp_token"] && !empty($token) && !empty($_SESSION["guest_progress"][$token]))
    $search_params = $_SESSION["guest_progress"][$token];

if ($user_id && (!empty($late_payment_plans["image_search"]) || !empty($late_payment_plans["general_search"]))) {
    $flg_late_payment_plans = false;
    if (!empty($late_payment_plans["general_search"]) && ($search_params["type"] == SEARCH_TYPE_EMAIL || $search_params["type"] == SEARCH_TYPE_USERNAME || $search_params["type"] == SEARCH_TYPE_PHONE || $search_params["type"] == SEARCH_TYPE_NAME))
        $flg_late_payment_plans = true;
    if (!empty($late_payment_plans["image_search"]) && $search_params["type"] == SEARCH_TYPE_IMAGE)
        $flg_late_payment_plans = true;
...
if (array_key_exists(PLAN_UNLIMITED_GENERAL_5_DAY_TRIAL, $user_plans) || array_key_exists(PLAN_UNLIMITED_3_DAY_SS, $user_plans))
    $social_search_plan = true;
if (array_key_exists(PLAN_UNLIMITED_IMAGE_5_DAY_TRIAL, $user_plans) || array_key_exists(PLAN_UNLIMITED_3_DAY_RIS, $user_plans))
    $image_search_plan = true;

if (empty($user_id))
    $no_res_categorykey = "guest";
else if (!empty($social_search_plan) && !empty($image_search_plan))
    $no_res_categorykey = "members_ss_ris";
else
    $no_res_categorykey = "members_ss";
...
if (SEARCH_TYPE_IMAGE == $search_params["type"]) {
    ...
    if ($user_id && User::has_tokens_for_search()) {
        ...
        if (!empty($search_params["images"]["tmp_name"][0]) && (Search::validate_image_file($search_params["images"]["tmp_name"][0]))) {
            ...
            $redirect_page = RELATIVE_URL . "ris-membership-levels/";
    ...
    if ((!defined("DEV_SKIP_PROGRESS_PAGE") || empty(DEV_SKIP_PROGRESS_PAGE)) && isset($_SERVER["HTTP_REFERER"]) && preg_match("/socialcatfish.com\/search\//i", $_SERVER["HTTP_REFERER"])) {
        ...
        if ($singular == "new")
            SYSTEM::redirect(PAGE_URL_GUEST_PROGRESS . "?token={$token}");
    ...
    */
    ...
    if ($search["result_count"]) {
        ...
        if ($search["result_count"] == 1 && empty($search_params["person_id"])) {
            ...
            if (empty($search["results"][0]["id"])) {
                ...
                $person_id = ((isset($search_data["tpd_id"]) && !empty($search_data["tpd_id"])) ? $report_page_id_flag[$search_data["tpd_id"]] : "") . $search["results"][0]["id"];
            ...
            if (!$user_id) {
                ...
                $_SESSION["guest_progress"][$token] = $search_params;
                if ($singular == "new")
                    $redirect_page = PAGE_URL_GUEST_PROGRESS . "?token={$token}";
        ...
        if (!empty($search_params["sid"])) {
            ...
            if ($_SESSION["last_search_params"]["email"]) {
                if (!empty($_SESSION["last_search_email"]))
                    unset($_SESSION["last_search_email"]);
                $last_search_email = $_SESSION["last_search_params"]["email"];
                foreach ($search["results"] as $key => $search_result) {
                    if (empty($search_result["name"]))
                        continue;
    ...
    if (!empty($search_data["params"]))
        $_SESSION["last_search_params"] = $search_data["params"];
    if (!empty($search_data["type"]))
        $_SESSION["last_search_type"] = $search_data["type"];
    if (!empty($search_data["cached_data"]["cache_id"]))
        $_SESSION["last_search_id"] = $search_data["cached_data"]["cache_id"];
    ...
    if ($user_id) {
        ...
        if (SYSTEM::checkNullEmpty($search_data["params"]["state"])) {
            ...
            if ($plan_id == 0) {
                ...
                if (!$records) {
                    ...
                    if ($user_has_criminal_tokens_available) {
                        $purchase_status = 1;
                        $subscibed       = true;
                        } else {
                        $purchase_status = 0;
        ...
        if (isset($_POST["search_type"]) && $_POST["search_type"] >= 0)
            Action_log::save_log("search:" . $_POST["search_type"], $baseline_track["start_time"], print_r($search_params, true));
        if (1 == $search["result_count"]) {
            ...
            if (!empty($_SESSION["privacy_lock_first_visit"]))
                unset($_SESSION["privacy_lock_first_visit"]);

            $include_map_scripts = true;

            if (!empty($_SESSION["user"]["pre_redesign_user"])) {

                if (empty($_SESSION["user"]["newlook_popup"]))
                    $include_popup[] = "new_look";
            ...
            if (!$charge_token || User::has_tokens_for_search()) {
                ...
                if ($charge_token)
                    User::add_remove_tokens($user_id, [$search_types[$search_data["type"]] => 1], "remove");
                ...
                if ($tpd_report) {
                    ...
                    foreach ($result["urls"] as $_index => $_link) {
                        ...
                        if (isset($filtered_urls[$check_url]))
            ...
            if (!empty($unclaimed_money_result))
                $include_popup[] = "unclaimed_money_popup";
            if (empty($redirect_page) && empty($search_params["person_id"]) && SEARCH_TYPE_PHONE != $search_data["type"])
                System::redirect(BASE_URL . "person/" . SYSTEM::sanitize($result["name"]) . "-{$result["id"]}/");
            ...
            if (!empty($result["record_dates"]["locations"])) {
                foreach ($result["record_dates"]["locations"] as $locationID => $location) {
                    ...
                    for ($i = count($location_names) + 1; $i > -1; --$i) {
                        if (!empty($location_names[$i]))
                            $locationStat = LOCATIONS::getStatByStateName(trim($location_names[$i]));
            ...
            // CSI-5307 end.
            if (isset($remove_head_foot))
                $remove_head_foot = false;
    ...
    if (1 < $search["result_count"] || ($search["result_count"] && SEARCH_TYPE_PHONE == $search_data["type"])) {
        ...
        if (preg_match("/([0-9]{3})([0-9]{3})([0-9]{4})\$/m", preg_replace("/[^0-9]/", "", $search_query_text), $formatted_phone_number))
            $formatted_phone_number = implode("-", array_slice($formatted_phone_number, 1));
        else
            $formatted_phone_number = $search_query_text;
...
if (isset($_POST["search_type"]) && $baseline_track["save"] == false && $_POST["search_type"] >= 0)
    Action_log::save_log("search:" . $_POST["search_type"], $baseline_track["start_time"], print_r($search_params, true));
...
if (!empty($search["results"])) {
    ...
    if (empty($squery_state))
        $squery_state = array_search(strtolower($_squery_state), array_map('strtolower', $list_of_states));
    $squery_city = SYSTEM::request("scity", "");
    if (!empty($squery_city)) {

        if (empty(Search::check_if_city_exists_in_state($squery_state, $squery_city)))
            $squery_city = "";
    ...
    foreach ($search["results"] as $_result_k => $_result) {
        ...
        if (!empty($squery_state) && empty($squery_city)) {
            if (!array_key_exists($squery_state, $_result["states"]))
                unset($search["results"][$_result_k]);
            }
        ...
        foreach ($single_results_field_list_for_counts as $field_name => $field_caption) {
            ...
            if (!isset($field_counts[$field_name]))
        ...
        if (!empty($_result["name"]) && !preg_match("/^[0-9\(\)\- ]+\$/", $_result["name"]))
            $field_counts["names"]++;
    ...
    if (!empty($search["cities"])) {
        foreach ($search["cities"] as $_search_filtered_city => $_search_filtered_count) {
            ...
            if (!empty($squery_state)) {
                if ($squery_state == $city_state)
                    $search_filtered_cities[$_search_filtered_city] = $_search_filtered_count;
                else
                    $search_other_cities[$_search_filtered_city] = $_search_filtered_count;
                }

            if ($_search_filtered_count > $create_link_if_results_morethan) {

                if (!empty(Search::check_if_city_exists_in_state($city_state, $city_city)))
                    $create_city_links[] = $city_city;
...
if (!empty($search["meta"]["names"]))
    $field_counts["names"] += count($search["meta"]["names"]);

// Limit to 3 images
$results_images = array_slice($results_images, 0, 3);
foreach ($results_images as $_index => $img)
    $results_images[$_index] = SCF::imgcdn_url($img);
...
if (!empty($search_data["cached_data"]["query"])) {
    $velocity = Search::get_search_velocity($search_data["cached_data"]["query"]);
    } else if (!empty($result["name"]))
    $velocity = Search::get_search_velocity($result["name"]);
